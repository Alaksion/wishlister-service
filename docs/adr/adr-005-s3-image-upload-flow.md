...
- `itemId` is generated by the create use case as a UUID before repository insertion. It is separate from the repository-generated database primary key so the application layer does not depend on MongoDB's `ObjectId` generation.
