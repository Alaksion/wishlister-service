...
| Field | Type | Notes |
|-------|------|-------|
| id | string (UUID) | Generated by the create use case before persistence |
| userId | string | Ref to User; strict ownership |
...
- Images are uploaded to S3 using a two-phase staging flow:
  1. The use case generates a UUID `itemId` before repository insertion so staging and final keys share the same identifier.
